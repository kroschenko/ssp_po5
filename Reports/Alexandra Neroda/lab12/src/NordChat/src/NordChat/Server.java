...
public class Server {
    ...
    private synchronized void broadcast(String messageLf, int recipientId) {
        ...
        // to deleting disconected guest
        ClientThread clientThread = clients.get(recipientId);
        if (!clientThread.writeMessage(messageLf)) {
...
//                String messageLf = time + " " + message + "\n";
                switch (clientMessage.getType()) {                      // Switcher of the types
                    ...
                    case Message.PRIVATE_MESSAGE:
                        String private_message = time + " " + username+ " -> "+ clients.get(clientMessage.getRecipient()).username + ": " + message + "\n";
                        writeMessage(private_message);
                        broadcast( private_message, clientMessage.getRecipient());
                        break;
                    case Message.LOGOUT:
                    ...
                    case Message.WHOISIN:
                        ...
                        break;
                    case Message.WHO_IS_IN_TO_PRIVATE:
                        System.out.println("Message.WHO_IS_IN_TO_PRIVATE");
                        StringBuilder username = new StringBuilder();
                        username.append("DLG:");
                        for (int i = 0; i < clients.size(); ++i) {
                            ClientThread clientThread = clients.get(i);
                            username.append((i+1)+". "+ clientThread.username + (i == clients.size() -1 ? "":  ";"));
                        }
                        writeMessage(username.toString());
                        break;
                }
            }
