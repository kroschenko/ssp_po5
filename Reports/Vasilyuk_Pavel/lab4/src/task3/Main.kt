...
fun main(args: Array<String>) {
    val client1: Payments.Client = Payments.Client()
    val client2: Payments.Client = Payments.Client()
    val good1 = Sum()
    good1.setSum = 2000
...
open class Payments {
    open class User { // some functional
    }

    class Client : User() {
        private val account: Account
        private val card: CCard
        fun Pay(good: Sum?) // using Card
        {
            if (good != null) {
                card.Pay(good)
            }
...
// any Card has Account
class CCard (var Account: Account) {
